...
import { fetchClasses, fetchChapters, generateQuestions, fetchCheatsheet } from '../api/quizApi';
...
const QuizMode = () => {
  ...
  useEffect(() => {
    ...
    fetchClasses(selectedSubject)
      ...
      .catch(() => setError('Failed to load classes. Is the quiz server running?'))
  ...
  const currentStep = !selectedSubject ? 1 : !selectedClass ? 2 : selectedChapters.length === 0 ? 3 : 4;
  ...
  const handleRevise = async () => {
    if (!selectedSubject || !selectedClass || selectedChapters.length === 0) return;
      ...
      {sheet.formulas?.length > 0 && (
        <div className="cheatsheet-section">
          <h3 className="cheatsheet-section-title">
            ...
            Formulas & Rules
            <span className="cheatsheet-section-badge">{sheet.formulas.length}</span>
          </h3>
      ...
      {sheet.strategies?.length > 0 && (
        <div className="cheatsheet-section">
          <h3 className="cheatsheet-section-title">
            <span className="cheatsheet-section-icon">💡</span>
            Strategies & Tricks
            <span className="cheatsheet-section-badge">{sheet.strategies.length}</span>
          </h3>
          <div className="cheatsheet-cards">
            {sheet.strategies.map((s, i) => (
              <div className="cheatsheet-card strategy-card" key={i}>
                <div className="cheatsheet-card-header"><MarkdownWithMath content={s.name} /></div>
                <div className="cheatsheet-card-row">
                  <span className="cheatsheet-card-label">Trick</span>
                  <MarkdownWithMath content={s.trick} />
                </div>
                <div className="cheatsheet-card-row why-row">
                  <span className="cheatsheet-card-label">Why students miss this</span>
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.detail || 'Failed to generate questions. Please try again.'
      );
    } finally {
      setGenerating(false);
    }
    ...
    <div className={`quiz-mode-wrapper${isDark ? ' dark-mode' : ''}`}>
      ...
      >
        ...
        <nav className="quiz-breadcrumb">
          <button className="quiz-breadcrumb-link" onClick={() => navigate('/student-dash')}>
            Dashboard
          </button>
        ...
        <div className="quiz-mode-header">
          <h1>Test Prep</h1>
          <p>Select your class and chapters, then challenge yourself with AI-generated MCQ questions</p>
          <button className="quiz-prev-path-btn" onClick={handlePrevLearningPath}>
          ...
          <div className="quiz-steps">
            {[
              ...
              <React.Fragment key={step.num}>
                {i > 0 && (
                  <div className={`quiz-step-line ${currentStep > step.num - 1 ? 'completed' : ''}`} />
                )}
                <div
                  className={`quiz-step ${
                    currentStep === step.num ? 'active' : currentStep > step.num ? 'completed' : ''
                  }`}
                >
          ...
          <div className="quiz-glass-card">
            <div className="quiz-loading-overlay">
              ...
              <span className="quiz-loading-subtext">
                AI is crafting bridge-diagnostic questions for {selectedChapters.length} chapter{selectedChapters.length > 1 ? 's' : ''}
              </span>
            </div>
          </div>
        ) : (
              ...
              <div className="quiz-subject-grid">
                {SUBJECTS.map((subj) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.96 }}
                  >
                    <span className="quiz-subject-icon">{subj === 'PHYSICS' ? '⚛️' : '📐'}</span>
                    <span>{subj}</span>
                  </motion.button>
                ))}
              </div>
            ...
            <div className="quiz-glass-card">
              ...
              {loadingClasses ? (
                <div className="quiz-empty-state">
                  <div className="quiz-spinner" style={{ margin: '0 auto', width: 32, height: 32 }} />
                </div>
              ) : (
                <select
              ...
              {selectedClass && (
                ...
                >
                  <div className="quiz-section-label">
                    ...
                    {chapters.length > 0 && (
                      <button className="quiz-select-all-btn" onClick={toggleAll}>
                      ...
                      {selectedChapters.map((ch) => (
                        <span className="quiz-selected-tag" key={ch}>
                          ...
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); removeChapter(ch); }}
                    ...
                    <div className="quiz-chapter-grid">
                      {filteredChapters.map((ch) => (
                        <motion.button
                          ...
                          whileTap={{ scale: 0.96 }}
                        >
                          <span className={`quiz-chip-check ${selectedChapters.includes(ch) ? 'visible' : ''}`}>✓</span>
                          <span className="quiz-chip-text">{ch}</span>
                        </motion.button>
                      ))}
                      {filteredChapters.length === 0 && chapterFilter && (
                        <div className="quiz-empty-state" style={{ width: '100%' }}>
                ...
                >
                  ...
                  <div className="quiz-config-row">
                    <div className="quiz-config-slider">
                      <label className="quiz-form-label">
                        <span className="label-icon">🎯</span> Questions Per Chapter
                      </label>
                  ...
                  <div className="quiz-summary-grid">
                    <div className="quiz-summary-card">
                      <div className="quiz-summary-card-icon">{selectedSubject === 'PHYSICS' ? '⚛️' : '📐'}</div>
                    ...
                    <div className="quiz-summary-card">
                      <div className="quiz-summary-card-icon"></div>
                      <div className="quiz-summary-card-data">
                        <div className="summary-val">{selectedChapters.length}</div>
                  ...
                  <div className="quiz-action-buttons">
                    ...
                    >
                      {loadingCheatsheet ? (
                        <>
                          <div className="quiz-spinner" style={{ width: 18, height: 18, borderWidth: 2 }} />
                          Loading Cheatsheet...
                        </>
                      ) : (
                        <>
        ...
        <AnimatePresence>
          {showPrevQuizzes && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => { setShowPrevQuizzes(false); setSelectedQuiz(null); }}
            >
              <motion.div
              ...
              >
                <div className="cheatsheet-header">
                  <div className="cheatsheet-title-row">
                    ...
                    <div>
                      <h2>{selectedQuiz ? selectedQuiz.name : 'Previous Learning Paths'}</h2>
                      <p>
                        {selectedQuiz
                ...
                <div className="cheatsheet-body">
                  {loadingPrevQuizzes ? (
                    <div className="quiz-empty-state">
                      <div className="quiz-spinner" style={{ margin: '0 auto', width: 36, height: 36 }} />
                        ...
                        {prevQuizzes.map((quiz) => (
                          ...
                          >
                            <div className="prev-quiz-item-top">
                              <span className="prev-quiz-name">{quiz.name}</span>
                              {quiz.analysis?.score_pct != null && (
                                <span className={`prev-quiz-score ${quiz.analysis.score_pct >= 70 ? 'good' : quiz.analysis.score_pct >= 40 ? 'mid' : 'low'}`}>
                            ...
                            <div className="prev-quiz-item-meta">
                              ...
                              {quiz.analysis?.correct != null && (
                                <>
                                  <span>&middot;</span>
                                  <span>{quiz.analysis.correct}/{quiz.analysis.total} correct</span>
                                </>
                              )}
                            </div>
                            {(!quiz.questions || quiz.questions.length === 0) && (
                              <span className="prev-quiz-no-q">No questions available</span>
                            )}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="learning-mode-questions">
                        ...
                        return (
                          <div className="learning-q-card" key={idx}>
                            <div className="learning-q-header">
                              ...
                              <span className="learning-q-chapter">{q.chapter}</span>
                              {q.difficulty && (
                                <span className={`learning-q-diff ${q.difficulty.toLowerCase()}`}>{q.difficulty}</span>
                              )}
                            </div>
                            ...
                            <div className="learning-q-options">
                              {Object.entries(q.options).map(([key, val]) => {
                                ...
                                if (answered) {
                                  if (key === q.correct_answer) optClass += ' correct';
                                  else if (key === answered && !isCorrect) optClass += ' wrong';
                                ...
                                return (
                                  <button
                                    key={key}
                                    className={optClass}
                                    onClick={() => handleLearningAnswer(idx, key)}
                                    disabled={!!answered}
                                  >
                                    <span className="learning-opt-key">{key}</span>
                            ...
                            {answered && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, height: 'auto' }}
                              >
                                <span className="learning-feedback-icon">{isCorrect ? '✓' : '✗'}</span>
                                <div className="learning-feedback-text">
                                  <strong>{isCorrect ? 'Correct!' : `Incorrect — Answer: ${q.correct_answer}`}</strong>
                                  {q.solution && (
                      ...
                      <div className="learning-progress-bar">
                        <div className="learning-progress-fill" style={{ width: `${(Object.keys(learningAnswers).length / selectedQuiz.questions.length) * 100}%` }} />
                      </div>
                      <div className="learning-progress-text">
                        {Object.keys(learningAnswers).length} / {selectedQuiz.questions.length} answered
                        {Object.keys(learningAnswers).length === selectedQuiz.questions.length && (
                          <span className="learning-score">
                            &nbsp;&middot;&nbsp;Score: {Object.entries(learningAnswers).filter(([i, a]) => a === selectedQuiz.questions[i].correct_answer).length}/{selectedQuiz.questions.length}
                          </span>
                        )}
                      </div>
                    </div>
                ...
                <div className="cheatsheet-footer">
                  {selectedQuiz ? (
                    <>
                      <button className="cheatsheet-done-btn" onClick={() => { setSelectedQuiz(null); setLearningAnswers({}); }}>
                        Back to List
                      </button>
                      <button className="cheatsheet-done-btn" onClick={() => { setShowPrevQuizzes(false); setSelectedQuiz(null); }}>
                        Close
                      </button>
                    </>
                  ) : (
                    <button className="cheatsheet-done-btn" onClick={() => setShowPrevQuizzes(false)} style={{ flex: 1 }}>
                      Close
                    </button>
                  )}
                </div>
              ...
              >
                  ...
                  <div className="cheatsheet-title-row">
                    ...
                    <div>
                      ...
                      <p>
                        ...
                        {cheatsheetData.total_sheets} chapter{cheatsheetData.total_sheets > 1 ? 's' : ''}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="cheatsheet-body">
                  {cheatsheetData.sheets.map((sheet, idx) => (
                    <div className="cheatsheet-chapter" key={idx}>
                      ...
                      >
                        <span className="cheatsheet-chapter-num">Ch {sheet.chapter_num}</span>
                        <span className="cheatsheet-chapter-name">{sheet.chapter}</span>
                        <span className="cheatsheet-chapter-meta">
                          {sheet.formulas?.length || 0} formulas &middot; {sheet.strategies?.length || 0} strategies
                        </span>
                        <span className={`cheatsheet-chevron ${expandedSheets[idx] ? 'open' : ''}`}>&#9662;</span>
                      </button>
                ...
                <div className="cheatsheet-footer">
                  ...
                  <button
                    ...
                    onClick={() => { setShowCheatsheet(false); handleGenerate(); }}
                    disabled={generating}
                  >
                    {generating ? (
                      <>
